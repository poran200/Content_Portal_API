...
import com.gft.manager.dto.UserRegDto;
import com.gft.manager.exception.*;
import com.gft.manager.model.CustomUserDetails;
import com.gft.manager.model.PasswordResetToken;
...
public class AuthService {
     ...
     */
    public Optional<User> registerUser(UserRegDto userRegDto, RoleName roleName) {
        String newRegistrationRequestEmail = userRegDto.getEmail();
     ...
     */
    public Optional<RefreshToken> createAndPersistRefreshTokenForUser(Authentication authentication) throws UserNotFoundException {
        User currentUser = (User) authentication.getPrincipal();
        refreshTokenService.findByUserId(currentUser.getId())
                        .map(RefreshToken::getId)
                                .ifPresent(refreshTokenService::deleteById);

        RefreshToken refreshToken = refreshTokenService.createRefreshToken();
        logger.info("Current User "+currentUser.getId());
        refreshToken.setUserId(currentUser.getId());
...
//                    userDeviceService.verifyRefreshAvailability(refreshToken
                ...
                .map(RefreshToken::getUserId).map(this::generateTokenFromUserId))
