...
import com.gft.manager.event.*;
import com.gft.manager.exception.*;
import com.gft.manager.model.CustomUserDetails;
import com.gft.manager.model.DeviceType;
import com.gft.manager.model.RoleName;
import com.gft.manager.model.payload.*;
import com.gft.manager.model.token.EmailVerificationToken;
import com.gft.manager.model.token.RefreshToken;
import com.gft.manager.util.UrlConstrains;
import com.gft.manager.security.JwtTokenProvider;
import com.gft.manager.service.impl.AuthService;
import com.gft.manager.service.impl.RefreshTokenService;
...
public class AuthController {
    ...
    @Autowired
    public AuthController(AuthService authService, JwtTokenProvider tokenProvider, RefreshTokenService refreshTokenService, ApplicationEventPublisher applicationEventPublisher, UserService userService) {
    ...
    public ResponseEntity authenticateUser(@Parameter(description = "The LoginRequest payload") @Valid @RequestBody LoginDto loginDto,
        ...
        String refreshToken = authService.createAndPersistRefreshTokenForUser(authentication)
        ...
        addCookie(jwtToken,refreshToken,response);
        return ResponseEntity.ok().body(loginSussesResponse);

    }

    private void addCookie(String token,String refreshToken, HttpServletResponse response) {
    ...
    private UserResponse getUserResponse(CustomUserDetails customUserDetails) {
        ...
        UriComponentsBuilder profileLink = ServletUriComponentsBuilder.fromCurrentContextPath().path(UrlConstrains.ProfileManagement.ROOT + "/" + userResponse.getEmail());
    ...
    private LoginRequest getLoginRequest(LoginDto loginDto) {
        ...
        loginRequest.setUserEmail(loginDto.getEmail());
...
//    @DataValidation
    ...
    public ResponseEntity refreshJwtToken(@Parameter(description = "The TokenRefreshRequest payload")
                                              @Valid @RequestBody TokenRefreshRequest tokenRefreshRequest,HttpServletResponse response) {

        return authService.refreshJwtToken(tokenRefreshRequest)
                .map(updatedToken -> {
                    String refreshToken = tokenRefreshRequest.getRefreshToken();
                    addCookie(updatedToken,refreshToken,response);
